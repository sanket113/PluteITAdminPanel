  ...
  onValue(categoriesRef, (snapshot) => {
    ...
    Object.entries(categories).forEach(([catId, category]) => {
      if (catId !== categoryId) {
        ...
        if (category.items) {
          Object.entries(category.items).forEach(([relatedItemId, relatedItem]) => {
            ...
            if (currentRelatedItems[category.title] && currentRelatedItems[category.title][relatedItemId]) {
...
function saveItemDetails() {
  ...
  document.querySelectorAll(".category-container").forEach((categoryDiv) => {
    ...
    const selectedItems = Array.from(categoryDiv.querySelectorAll(".category-checkbox:checked"));

    selectedItems.forEach((checkbox) => {
      const selectedItemId = checkbox.value;
      const selectedItemName =
        window.categoriesData[selectedCategoryId]?.items[selectedItemId]?.name || "";
      ...
      if (categoryName && selectedItemName) {
        ...
        relatedUpdates[`categories/${categoryId}/items/${itemId}/relatedItemsByCategory/${categoryName}/${selectedItemId}`] =
          selectedItemName;
        relatedUpdates[
    ...
    const previouslySelected = window.categoriesData[selectedCategoryId]?.items || {};
    Object.keys(previouslySelected).forEach((relatedItemId) => {
      if (!selectedItems.some((checkbox) => checkbox.value === relatedItemId)) {
        relatedUpdates[`categories/${categoryId}/items/${itemId}/relatedItemsByCategory/${categoryName}/${relatedItemId}`] = null;
        relatedUpdates[`categories/${selectedCategoryId}/items/${relatedItemId}/relatedItemsByCategory/${window.categoriesData[categoryId].title}/${itemId}`] =
          null;
      }
    });
  });
